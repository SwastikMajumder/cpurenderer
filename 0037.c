...
void draw_buffer(SDL_Renderer *renderer, struct COLOR_MIX *frame_buffer){
    ...
    for (x=0; x<RADIUS*2; ++x){
    	for(y=0; y<RADIUS*2; ++y){
    		bytes[x*32+y*4] = frame_buffer[x * 8 + y].RED;
    		bytes[x*32+y*4 + 1] = frame_buffer[x * 8 + y].GREEN;
    		bytes[x*32+y*4 + 2] = frame_buffer[x * 8 + y].BLUE;
    		bytes[x*32+y*4 + 3] = 255;
    ...
    for (x=0; x<2000*500; x+=4){
    	bytes[x] = 255;
    	bytes[x+1] = frame_buffer[index].BLUE;
    	bytes[x+2] = frame_buffer[index].GREEN;
    	bytes[x+3] = frame_buffer[index].RED;
    }*/
