...
int main(int argc, char *argv[]) {
	...
	struct TRIANGLE_3D_FLOAT *list = malloc(5 * 12 * 2 * 6 * sizeof(struct TRIANGLE_3D_FLOAT));
	
	int index = new_index(list, 5 * 12 * 2 * 6);
	
	struct POINT_3D_FLOAT location = {.X = -200.0f, .Y = 0.0f, .Z = 250.0f };
	/*
	obj_cuboid_0001(list, index, &location, 1, 25.0f, 25.0f, 25.0f);
	fill_color_cuboid_0001(list, index);
	*/
	obj_wall_0001(list, index, &location, 5, 5, 255, 0, 0);
	
	zbuffer(list, 5 * 12 * 2 * 6, frame_buffer);
	draw_buffer(renderer, frame_buffer);
	
	delete_index(list, index, 5 * 12 * 2 * 6);
