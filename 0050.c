	...
	for (i=0; i<RADIUS*2; ++i){
		for (j=0; j<RADIUS*2; ++j){
			...
			for (k=0; k<list_length; ++k){
				...
				char str[80];
sprintf(str, "%3.3f", list[k].TRIANGLE[0].Z);
SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, str, str, NULL);
				if (list[k].TRIANGLE[0].Z > (float)Z_LIMIT && list[k].TRIANGLE[1].Z > (float)Z_LIMIT && list[k].TRIANGLE[2].Z > (float)Z_LIMIT){
					if (
...
int main(int argc, char *argv[]) {
	...
	*/
	...
	obj_wall_0002(list, index + CTRIANGLE/2, &location, 2, 2, 255, 0, 0);
	//rotate_camera(list, index, CTRIANGLE, 0.0f, (10 * M_PI)/180, 0.0f);
