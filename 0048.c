...
SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, str, str, NULL);*/
		...
		fill_color(list, index + 12 * (i * 2 * (wall_length + 1)), 12, (c1 = !c1) ? red>>1 : red, (c1 = !c1) ? green>>1 : green, (c1 = !c1) ? blue>>1 : blue);
		for (j=0; j<wall_length; ++j){
			obj_cuboid_0001(list, index + 12 * (i * 2 * (wall_length + 1) + j * 2 + 1), x + (j * 50.0f), y + 25.0f * i * 2, z, 1, 50.0f, 25.0f, 25.0f);
			fill_color(list, index + 12 * (i * 2 * (wall_length + 1) + j * 2 + 1), 12, (c2 = !c2) ? red>>1 : red, (c2 = !c2) ? green>>1 : green, (c2 = !c2) ? blue>>1 : blue);
			obj_cuboid_0001(list, index + 12 * (i * 2 * (wall_length + 1) + j * 2 + 2), x + (j * 50.0f) + 25.0f, y + 25.0f * (i * 2 + 1), z, 1, 50.0f, 25.0f, 25.0f);
			fill_color(list, index + 12 * (i * 2 * (wall_length + 1) + j * 2 + 2), 12, (c1 = !c1) ? red>>1 : red, (c1 = !c1) ? green>>1 : green, (c1 = !c1) ? blue>>1 : blue);
		}
...
int main(int argc, char *argv[]) {
	...
	struct TRIANGLE_3D_FLOAT *list = malloc(CTRIANGLE * sizeof(struct TRIANGLE_3D_FLOAT));
	
	//for (i=0; i<MAX)
	
	int index = new_index(list, CTRIANGLE);
	
	struct POINT_3D_FLOAT location = {.X = -200.0f, .Y = 0.0f, .Z = 250.0f };
	/*
	obj_cuboid_0001(list, index, 50.0f, 0.0f, 250.0f, 1, 50.0f, 25.0f, 25.0f);
	fill_color(list, index, 12, 255, 0, 0);
	
	obj_cuboid_0001(list, index + 12, 100.0f, 0.0f, 250.0f, 1, 50.0f, 25.0f, 25.0f);
	fill_color(list, index + 12, 12, 255>>2, 0, 0);
	*/
	obj_wall_0001(list, index, &location, 2, 2, 255, 0, 0);
	
	zbuffer(list, CTRIANGLE, frame_buffer);
	draw_buffer(renderer, frame_buffer);
	
	delete_index(list, index, CTRIANGLE);
